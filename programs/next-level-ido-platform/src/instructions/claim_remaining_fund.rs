...
#[derive(Accounts)]
pub struct ClaimRemainingFund<'info> {
    #[account(
        mut,
        seeds = [
...
pub fn exe(ctx: Context<ClaimRemainingFund>) -> Result<()> {
    let remaining_fund = ctx.accounts.ido_user.remaining_amount;

    ctx.accounts.ido_user.remaining_amount = 0;
    ctx.accounts.ido_user.deposit_amount = ctx
        .accounts
        .ido_user
        .deposit_amount
        .checked_sub(remaining_fund)
        .unwrap();
    ...
    token::burn(cpi_ctx, remaining_fund)?;
    ...
    token::transfer(cpi_ctx, remaining_fund)?;
